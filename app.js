...
function getNextSplitSession(){
  ...
  if(!split||!split.sessions.length)return null;
  for(let i=workouts.length-1;i>=0;i--){
    const idx=split.sessions.indexOf(workouts[i].name);
    if(idx!==-1){
      const nextIdx=(idx+1)%split.sessions.length;
      return{name:split.sessions[nextIdx],dayIndex:nextIdx,total:split.sessions.length,splitName:split.name,sessions:split.sessions};
    }
  }
  return{name:split.sessions[0],dayIndex:0,total:split.sessions.length,splitName:split.name,sessions:split.sessions};
}
function startSplitSession(){
  const next=getNextSplitSession();
  if(!next)return;
  const ti=templates.findIndex(t=>t.name===next.name);
...
function renderSplitCard(){
  ...
  return`<div style="background:var(--bg2);border:1px solid var(--border);border-radius:var(--r2);padding:14px 16px;margin-bottom:16px">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center">
      <div>
        <div style="font-size:10px;color:var(--text3);letter-spacing:.4px;text-transform:uppercase;margin-bottom:2px">Next up</div>
        <div style="font-size:17px;font-weight:700;letter-spacing:-.2px">${esc(next.name)}</div>
      </div>
...
function renderExerciseBlock(ex,ei,isEditing){
  ...
  // ── EXERCISE NOTES ──
  const prevExNotes=getLastExerciseNotes(ex.name);
  const curExNotes=ex.notes||'';
  html+=`<div style="margin-top:10px">
    ${prevExNotes?`<div style="font-size:11px;color:var(--text2);background:var(--bg3);border-radius:var(--r);padding:6px 10px;margin-bottom:6px;line-height:1.5;border-left:2px solid var(--border2)"><span style="font-weight:600;color:var(--text3);text-transform:uppercase;letter-spacing:.4px;font-size:10px">Last time · </span>${esc(prevExNotes)}</div>`:''}
    <textarea class="set-note-input w-full" rows="1" placeholder="Exercise notes…"
      oninput="updateExNote(${ei},this.value);autoResizeNote(this)"
      onfocus="autoResizeNote(this)"
      style="font-size:12px;color:var(--text2)">${esc(curExNotes)}</textarea>
  </div>`;
...
function getLastExerciseNotes(name){
  for(let i=workouts.length-1;i>=0;i--){
    ...
    if(ex&&ex.notes&&ex.notes.trim())return ex.notes.trim();
  }
  return null;
}
function updateExNote(ei,value){
  const wo=editingWorkout?editingWorkout.workout:activeWorkout;
  if(!wo||!wo.exercises[ei])return;
  wo.exercises[ei].notes=value;
}
...
function startFromTemplate(i){
  ...
  const exercises=t.exercises.map(e=>{
    ...
    // Default: use template sets (preserves count, notes, dropset flags)
    let sets=e.sets&&e.sets.length?e.sets.map(s=>({...s,done:undefined})):[{}];
    if(hist.length){
...
function seedBassiTemplates(){
  ...
  const ts=Date.now();
  const u1='ss_bassi_u1', u2='ss_bassi_u2', u3='ss_bassi_u3';
  templates=[
    {name:'Legs (Quads)', exercises:[
      {id:ts+1,  name:'Standing Calf Raises',                                             type:'strength',   sets:[{},{},{}]},
      {id:ts+2,  name:'Smith Squat (heels elevated)',                                     type:'strength',   sets:[{},{},{notes:'Slow tempo'}]},
      {id:ts+3,  name:'Leg Press (feet low & narrow)',                                    type:'strength',   sets:[{},{},{}]},
      {id:ts+4,  name:'Barbell Back Squat',                                               type:'strength',   sets:[{},{}]},
      {id:ts+5,  name:'Leg Extension',                                                    type:'strength',   sets:[{},{},{notes:'[DROPSET] Partials at end'}]}
    ]},
    {name:'Upper', exercises:[
      {id:ts+10, name:'Incline DB Press (30°)',                                           type:'strength',   sets:[{},{},{}]},
      {id:ts+11, name:'Incline Smith Press (slightly higher angle, stretch and touch chest)', type:'strength', supersetId:u1, sets:[{},{},{}]},
      {id:ts+12, name:'Neutral Grip Pull-Ups',                                            type:'bodyweight', supersetId:u1, sets:[{},{},{}]},
      {id:ts+13, name:'Single Arm Lat Pulldown',                                          type:'strength',   supersetId:u2, sets:[{},{},{}]},
      {id:ts+14, name:'Machine Fly',                                                      type:'strength',   supersetId:u2, sets:[{},{},{}]},
      {id:ts+15, name:'Straight Arm Pulldown',                                            type:'strength',   supersetId:u3, sets:[{},{},{}]},
      {id:ts+16, name:'Row Machine Kelso Shrug',                                          type:'strength',   supersetId:u3, sets:[{},{},{}]}
    ]},
    {name:'Shoulders + Arms', exercises:[
      {id:ts+20, name:'Cable Lateral Raise',                                              type:'strength',   sets:[{},{},{}]},
      {id:ts+21, name:'Dumbbell Lateral Raise',                                           type:'strength',   sets:[{},{},{}]},
      {id:ts+22, name:'Cable Reverse Fly',                                                type:'strength',   sets:[{},{},{}]},
      {id:ts+23, name:'Overhead Cable Tricep Extension',                                  type:'strength',   sets:[{},{},{}]},
      {id:ts+24, name:'Tricep Pushdown',                                                  type:'strength',   sets:[{},{},{}]},
      {id:ts+25, name:'Incline DB Curl',                                                  type:'strength',   sets:[{},{},{}]},
      {id:ts+26, name:'Cable Curl',                                                       type:'strength',   sets:[{},{},{}]}
    ]},
    {name:'Legs (Hamstrings)', exercises:[
      {id:ts+30, name:'Calf Raises',                                                      type:'strength',   sets:[{},{},{}]},
      {id:ts+31, name:'Romanian Deadlift',                                                type:'strength',   sets:[{},{},{},{}]},
      {id:ts+32, name:'Seated Hamstring Curl (hold contraction, slow negative)',          type:'strength',   sets:[{},{},{}]},
      {id:ts+33, name:'Hip Thrusts (hold contraction)',                                   type:'strength',   sets:[{},{},{}]},
      {id:ts+34, name:'Leg Press',                                                        type:'strength',   sets:[{},{},{}]}
    ]},
    {name:'Push', exercises:[
      {id:ts+40, name:'Incline Flyes',                                                    type:'strength',   sets:[{},{},{}]},
      {id:ts+41, name:'Incline DB Press',                                                 type:'strength',   sets:[{},{},{}]},
      {id:ts+42, name:'Cable Fly (low to high)',                                          type:'strength',   sets:[{},{},{}]},
      {id:ts+43, name:'Cable Lateral Raise (behind the body)',                            type:'strength',   sets:[{},{},{}]},
      {id:ts+44, name:'DB Lateral Raise',                                                 type:'strength',   sets:[{},{},{}]},
      {id:ts+45, name:'DB Overhead Extension',                                            type:'strength',   sets:[{},{},{}]},
      {id:ts+46, name:'Dips',                                                             type:'bodyweight', sets:[{},{},{}]}
    ]},
    {name:'Pull', exercises:[
      {id:ts+50, name:'Neutral Grip Pull-Ups',                                            type:'bodyweight', sets:[{},{},{}]},
      {id:ts+51, name:'Single Arm Lat Pulldown (partials to failure, hold last rep)',     type:'strength',   sets:[{},{},{}]},
      {id:ts+52, name:'Straight Arm Pulldown',                                            type:'strength',   sets:[{},{},{}]},
      {id:ts+53, name:'Y-Raise',                                                          type:'strength',   sets:[{},{},{}]},
      {id:ts+54, name:'Preacher Curl',                                                    type:'strength',   sets:[{},{},{}]},
      {id:ts+55, name:'Hammer Curl',                                                      type:'strength',   sets:[{},{},{}]}
    ]}
  ];
  save();
}
